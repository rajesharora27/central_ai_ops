...
| `global-task-governance.md` | Standardizes the task hub workflow for repo changes. | Requires the plan -> tasks -> `npm run tasks:sync` -> implement sequence, defines task IDs/frontmatter/status rules, supports optional `related` links for sibling work, infers plan trails from shared `plan:` metadata, and requires completed tasks to move into `docs/tasks/completed/` with `docs/TODO.md` regenerated. |
